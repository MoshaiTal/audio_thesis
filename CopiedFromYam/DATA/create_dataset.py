...
class ReverbDataset(Dataset):
    ...
    def _parse_len_from_filename(self, path: str):
        m = re.search(r'\[len=(\d+)\]', os.path.basename(path))
        return int(m.group(1)) if m else None

    def _infer_valid_time_from_array(self, arr: np.ndarray) -> int:
        if arr.ndim == 3:
    ...
    def _get_valid_time(self, clean_path: str, clean_arr: np.ndarray) -> int:
    ...
    def _normalize_array(self, arr, valid_t: int):
    ...
    def reconstruct_from_splits(self, splits):
        ...
        if splits.ndim == 4:
            ...
            reshaped = reshaped[:, :self.final_freq_idx, :]
            pad_t = max(0, self.target_total_T - reshaped.shape[-1])
            reconstructed = np.pad(
                reshaped, ((0, 0), (0, 0), (0, pad_t)),
    ...
    def __getitem__(self, index):
        ...
        for i in range(1, self.mics_num + 1):
            file_path_reverb_new = (
                file_path_reverb[:file_path_reverb.find('[')] + f'ch{i}_' + file_path_reverb[file_path_reverb.find('['):]
            )
