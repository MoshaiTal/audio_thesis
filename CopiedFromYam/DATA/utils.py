...
def spectrogram_to_mel(spectrograms, sr=16000, n_mels=80, n_fft=400):
    """
    Convert a batch of spectrograms (STFT magnitude) to a batch of Mel spectrograms.

    Args:
        spectrograms (np.ndarray or torch.Tensor): Batch of spectrograms (batch_size, freq_bins, time_frames).
        sr (int): Sample rate (default: 16kHz for Whisper).
        n_mels (int): Number of Mel bins (default: 80 for Whisper).
        n_fft (int): FFT window size.

    Returns:
        np.ndarray or torch.Tensor: Log-Mel spectrograms with shape (batch_size, n_mels, time_frames).
    """
    ...
    if isinstance(spectrograms,list):
        ...
        for s in spectrograms:
            s = s.permute(0, 2, 1)[:, :201, :]
            is_torch = isinstance(s, torch.Tensor)
            if is_torch:
                s = s.cpu().numpy()
             # TODO-  change the whole model to generate spectrogram's like this instead of trim it
            mel_spectrograms = np.einsum('mf,bft->bmt', mel_filter, s)
            log_mel_spectrograms = librosa.power_to_db(mel_spectrograms, ref=np.max)
            # log_mel_spectrograms = np.clip(log_mel_spectrograms, -80, 0)  # Keep within Whisper's range
            # log_mel_spectrograms = (log_mel_spectrograms + 80) / 80
            if is_torch:
                log_mel_spectrograms = torch.tensor(log_mel_spectrograms, dtype=torch.float32).permute(0,2,1)

            mel_spectrograms_list.append(log_mel_spectrograms)
        return mel_spectrograms_list

    is_torch = isinstance(spectrograms, torch.Tensor)
    spectrograms = spectrograms.permute(0, 2, 1)[:, :201, :]
    if is_torch:
        spectrograms = spectrograms.cpu().numpy()
      # TODO-  change the whole model to generate spectrogram's like this instead of trim it
    mel_spectrograms = np.einsum('mf,bft->bmt', mel_filter,spectrograms)
    log_mel_spectrograms = librosa.power_to_db(mel_spectrograms, ref=np.max)
    # log_mel_spectrograms = np.clip(log_mel_spectrograms, -80, 0)  # Keep within Whisper's range
    # log_mel_spectrograms = (log_mel_spectrograms + 80) / 80
    if is_torch:
...
def normelize_mel_spec_for_whisper(log_mel_spectrogram,no_need1,no_need2):
    mean = log_mel_spectrogram.mean()
    std = log_mel_spectrogram.std()
    return ((log_mel_spectrogram - mean) / std).numpy()
