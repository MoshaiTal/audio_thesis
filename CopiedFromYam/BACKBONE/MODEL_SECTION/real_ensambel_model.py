...
class DecoderBlock(nn.Module):
    def __init__(self, in_filters5, num_classes, alpha):
        super().__init__()
        self.alpha=alpha
        self.num_classes=num_classes
        self.upsample6 = torch.nn.ConvTranspose2d(in_filters5, 32 * 8, kernel_size=(2, 2), stride=(2, 2))
        ...
        self.head = Conv2d_batchnorm(self.in_filters9, self.num_classes + 1, kernel_size=(1, 1), activation='None')

    def forward(self, x_multires5, x_multires4, x_multires3, x_multires2, x_multires1):
...
class MultiResUnet(torch.nn.Module):
    ...
    def __init__(self, input_channels, num_classes, alpha=1.67):
        super().__init__()
        self.num_heads=len(config['Model params']['heads_params'])*2
        self.alpha = alpha
        ...
        self.decoder=DecoderBlock(self.in_filters5, num_classes, alpha)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        outputs = self.decoder(x_multires5, x_multires4, x_multires3, x_multires2, x_multires1).squeeze(1)  # [B,H,W]
        return outputs

class EnsembleUNet(nn.Module):
    """
    Holds one `SingleHeadUNet` per quantile so models do *not* share weights.
    """
    def __init__(self, input_channels: int, num_classes: int):
        super().__init__()
        head_cfgs=config['Model params']['heads_params']
        self.models = nn.ModuleList(
            [MultiResUnet(input_channels, num_classes) for _ in range(len(head_cfgs)*2)]
        )

    def forward(self, x):
        # serial evaluation ⇒ lower peak VRAM
