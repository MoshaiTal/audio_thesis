...
def pad_freq_for_unet(x: torch.Tensor, target_freq: int = 256, pad_value: float = 0.0) -> torch.Tensor:
    if x.shape[-2] >= target_freq:
        return x
    return F.pad(x, (0, 0, 0, target_freq - x.shape[-2]), value=pad_value)


def pad_mask_freq_for_unet(mask: torch.Tensor, target_freq: int = 256) -> torch.Tensor:
    if mask.shape[-2] >= target_freq:
        return mask
    return F.pad(mask, (0, 0, 0, target_freq - mask.shape[-2]), value=0.0)
...
def infer_and_save_split(split: str, model, device, cfg: dict, output_tag: str) -> None:
    ...
    ds = loader.dataset
    norm_min = float(ds.norm_min)
    norm_max = float(ds.norm_max)
    pad_value = float(getattr(ds, "normalized_pad_value", 0.0))
    mics_num = int(cfg["General"]["mics_num"])
    out_root = make_mel_root(cfg, output_tag)
    model.eval()

    print(f"[INFO] split={split}, samples={len(ds)}, norm=({norm_min:.6f}, {norm_max:.6f})")
    ...
    for idx in range(len(ds)):
        ...
        reverb_t = pad_freq_for_unet(reverb_t, target_freq=256, pad_value=pad_value)
        mask_t = pad_mask_freq_for_unet(mask_t, target_freq=256)
        ...
        for ch in range(1, mics_num + 1):
            ...
            pred_norm = chunks_to_full_mel(outs[0].detach().cpu().numpy().astype(np.float32))
            ...
            for head, dic_params in cfg["Model params"]["heads_params"].items():
                ...
                lo_norm = chunks_to_full_mel(outs[int(head)].detach().cpu().numpy().astype(np.float32))
                hi_norm = chunks_to_full_mel(outs[-int(head)].detach().cpu().numpy().astype(np.float32))
                lo = denormalize_minus1_1(lo_norm, norm_min, norm_max)
                hi = denormalize_minus1_1(hi_norm, norm_min, norm_max)
                np.save(out_dir / f"{clean_stem}_ch{ch}_lower_alpha={alpha}.npy", lo.astype(np.float32))
                np.save(out_dir / f"{clean_stem}_ch{ch}_upper_alpha={alpha}.npy", hi.astype(np.float32))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate dereverb_mel outputs from a Whisper-supervised mel model.")
    parser.add_argument("--run_splits", default="val", help="Comma-separated: train,val,test,cal")
    parser.add_argument("--output_tag", required=True, help="Outputs go to dereverb_mel/calibrated_<output_tag>")
    args = parser.parse_args()

    cfg = load_config()
    if cfg["General"].get("DATA_TYPE") != "melspec":
        raise RuntimeError("This generator expects config['General']['DATA_TYPE'] == 'melspec'.")
